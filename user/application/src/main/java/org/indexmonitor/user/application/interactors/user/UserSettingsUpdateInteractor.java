...
class UserSettingsUpdateInteractor extends Interactor implements UserSettingsUpdateUseCase {
    private final Logger logger = LoggerFactory.getLogger(UserSettingsUpdateInteractor.class);
    ...
    public BaseResponse update(UserSettingsUpdateRequest request) {
        try
        {
            ...
            logger.debug(String.format("Failed to update settings for user with ID '%s'. Exception message: '%s'.", request.getId() == null ? "null" : request.getId(), e.getMessage()));
