...
@RestController
@RequestMapping("api/users/{userId}/roles/{roleId}")
@AllArgsConstructor
public class UserRolesRemoveController {
    private static String[] REQUIRED_ROLES = {"USER-MANAGER"};
    private static String[] REQUIRED_AUTHORITIES = {"UPDATE"};
    private final UserRolesRemoveUseCase userRolesRemoveUseCase;

    @DeleteMapping
    public ResponseEntity<BaseResponse> remove(@PathVariable(name = "userId") String userId, @PathVariable(name = "roleId") String roleId){
        AccessControlUtil.builder().hasAllRoles(REQUIRED_ROLES).hasAllAuthorities(REQUIRED_AUTHORITIES).validate();
        BaseResponse response = userRolesRemoveUseCase.remove(new UserRolesUpdateRequest(userId, roleId));
